...
\]
where \(\rho_u\) is the quantile score, or check loss. Thus, the CRPS is a natural score in the context of quantile estimation. For a posterior predictive sample \(z_t^{(1)},\ldots,z_t^{(S)}\), we approximate the integral using the empirical quantile grid \(u_s=s/(S+1)\), \(s=1,\ldots,S\). Let \(z_{t,(1)} \leq \cdots \leq z_{t,(S)}\) be the ordered sample values. Then
\[
