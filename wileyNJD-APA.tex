...
\end{bmatrix}.
\]
Let \(y_t^0 \equiv y_t^o\) denote the observation channel and \(y_t^s \equiv z_t^s\), \(s\in\mathcal{J}\), denote the retrospective-product channels. The corresponding compact form is:
\begin{alignat}{3}
...
Following \citet{barata2021_flex_quantile}, for each skewness parameter, we assign a truncated Student-t prior: \(\gamma^j \sim t_{(L, U)}(0, \phi^j, \nu^j)\), where $\phi^j$ and $\nu^j$ denote the scale and degrees of freedom hyperparameters, respectively, for \(1 \leq j \leq J\). For the scale parameters, we use inverse gamma priors: \(\sigma^j \sim IG(a_{\sigma^j}, b_{\sigma^j})\), with $a_{\sigma^j}$ and $b_{\sigma^j}$ as shape and scale hyperparameters. The latent trend-and-seasonal block is initialized as \(\boldsymbol{\theta}_0 \sim \mathcal{N}(\mathbf{m}_0, \mathbf{C}_0)\), with \(\mathbf{m}_0 \in \mathbb{R}^{p}\) and \(\mathbf{C}_0 \in \mathbb{R}^{p\times p}\). \textcolor{red}{What about observation level parameters $\gamma^0$ and $\sigma^0$? indexing above goes from $1 \leq j \leq J$ so should we include $j=0$? This may also need to be checked in Section 2.4 below. Also, for the initial states, do you need to initialize the discrepancy and transfer states? In other words, $\boldsymbol{\eta}_t \sim N(\mathbf{m}_0, \mathbf{C}_0)$?}
...
Our approach fits a separate state-space model for each target quantile level, which allows the seven quantile-specific models used in the application to be fitted in parallel. In the San Lorenzo application, the full quantile set can be refit on operational time scales once observations, retrospective products, forecast products, and forecast covariates have been staged; representative publication runs take about two hours 
\textcolor{red}{on a Linux server with 64 cores and 503 GiB of RAM. This is a combined wall-clock time because fitting and forecasting were not timed separately. The main computational  cost arises from repeated filtering and smoothing operations and increases with the length of the time series and the dimension of the latent state.}
%end-to-end in our computing environment. 
%These timings use the practical runtime fields \texttt{runtime\_sec\_total} and \texttt{runtime\_sec}, and are hardware- and implementation-dependent.
...
\]
where \(\rho_u\) is the quantile score, or check loss. Thus, the CRPS is a natural score in the context of quantile estimation. For a posterior predictive sample \(z_t^{(1)},\ldots,z_t^{(S)}\), \bs{\bf I think you mean samples of the predicted quantiles, not the predictive disttribution. So, you probably want to start by saying that our method produces samples of the quantiles.}we approximate the integral using the empirical quantile grid \(u_s=s/(S+1)\), \(s=1,\ldots,S\). Let \(z_{t,(1)} \leq \cdots \leq z_{t,(S)}\) be the ordered sample values. Then
\[
\widehat{\mathrm{CRPS}}(y_t,G_t)
= \frac{2}{S}\sum_{s=1}^S
\rho_{u_s}\!\left(y_t-z_{t,(s)}\right).
\]
The same calculation is used for synthesized posterior predictive samples and for raw forecast ensembles, with \(S\) equal to the number of available sample draws or ensemble members at that lead. If the predictive distribution is a point mass at \(x\), the CRPS reduces to \(|y-x|\); averaging this quantity across forecast cases gives the mean absolute error. We focus on CRPS and quantile check loss; probability integral transform (PIT) diagnostics are not emphasized because the validation design is based on a small number of carefully staged rolling origins. We also use CRPS for the preliminary selection of the transfer-persistence parameter \(\lambda\), as described in Subsection~\ref{subsec:specification}.
...
Our target series is the average daily natural water flow (in cubic feet per second \textcolor{red}{is it cubic feet or cubic meters? in the figure it appears as cubic meters}) of the San Lorenzo River at the Big Trees U.S. Geological Survey (USGS) gauging station in Santa Cruz, California (Figure \ref{fig:sanlorenzo}). The Big Trees location is characterized by minimal human intervention, so the recorded flow closely reflects natural river dynamics. Over the study window used in this article, the daily series covers May 29, 1987, through December 25, 2022, and contains 12,995 observations. 
The San Lorenzo River is a major water resource for the Santa Cruz area and is relevant to both flood response and longer-term water-supply planning. The record includes major flood episodes as well as prolonged dry conditions. The analysis and forecasting of lower quantiles are therefore relevant for drought conditions, whereas upper quantiles are relevant for flood risk.

\begin{figure}[htbp]
\centering
\includegraphics[width=450pt]{site_context_usgs.png}
\caption{
Observed USGS daily water flow on the $\log(1 + x)$ scale, with $x$ measured in m$^3$/s, May 29, 1987 through December 25, 2022. Horizontal dashed lines indicate NOAA-defined minor (16.5 ft, 503 cm) and major (21.76 ft, 663 cm) flooding thresholds, converted to the plotting scale. \textcolor{red}{Antonio, segun GPT: the reported flood thresholds—16.5 ft and 21.76 ft—are
  river-stage measurements, whereas the graph is on a discharge scale. A stage
  threshold cannot be placed on a discharge graph without a stage–discharge
  conversion. The paper must identify the rating curve or conversion used. If no
  conversion was performed, those horizontal lines should be removed. Esto es importante y si no estas seguro puedes simplemente eliminar las lineas horizontales y el texto relacionado a major y minor flooding que tengas en el resto del paper.
}
}
...
For prior hyperparameters (see Subsection~\ref{subsec:prioranddisc}), we specify \(\phi^j = 10^6\), \(\nu^j = 1\), and \(a_{\sigma^j} = b_{\sigma^j} = 10^{-6}\) for all \(1 \leq j \leq J\), \textcolor{red}{Should we add $j=0$?} reflecting diffuse prior beliefs. Sensitivity analyses for \(\phi^j \in \{10^1, 10^3, 10^6\}\) and \(b_{\sigma^j} \in \{10^{-1}, 10^{-3}, 10^{-6}\}\) showed no noticeable impact on model fit.
...
The transfer-persistence parameter \(\lambda\) was selected in a preliminary one-dimensional tuning step. To avoid confounding transfer persistence with state-evolution adaptability, the component discount factors were held fixed during this step. We considered the grid \(\lambda \in \{0.60,0.61,\ldots,0.99\}\) and evaluated candidate values using forecast-window CRPS for the synthesized predictive distribution at the representative December 25, 2022 cutoff, which has the longest pre-origin archive among the rolling-origin cases. For computational efficiency, this tuning used the median-quantile specification. The selected value was \(\lambda=0.97\), which was then held fixed across all quantile levels and cutoffs. For the forecast-window covariance prior, we set \(\epsilon\) \textcolor{red}{What is the value of $\epsilon$?} and \(c=1\); these settings stabilize short-horizon forecast propagation while preserving the covariance scale learned before the cutoff.
\textcolor{red}{The discount factor was optimized for the exAL-M-T1 model only, correct? Also the optimization of the discount factor for this model was based on the predictive distribution, correct? If so then this model is going to have an advantage with respect to competing models predictively because the other models do not have their discount values optimized predictively, right? Most important part is that the model is not evaluated with out-of-sample data only, since you used to to select the discount factor no?  In this sense this is not really like cross-validation. Let's discuss this when we meet.}

\subsection{Rolling-Origin Forecast Evaluation Design}

The evaluation uses out-of-sample predictions for five rolling-origin cutoff dates that span contrasting hydrological conditions and remain compatible with the archived retrospective and forecast products available from each agency. The retained origins include relatively low-flow windows as well as winter high-flow episodes, so the assessment covers different operating regimes, although it is not a continuous daily hindcast over the full post-2022 period. This rolling-origin design is the time-ordered analogue of cross-validation for the operational forecasting problem: each fold fixes a forecast origin, uses only information available at that origin, and scores the resulting predictive distribution against future USGS observations.

Constructing a fold requires more than shifting the cutoff date. The archived products come from evolving operational systems with different product versions, update frequencies, horizons, ensemble structures, and spatial supports. For each retained origin we rebuild a version-consistent bundle containing USGS observations, retrospective products, forecast products, and forecast-window covariates. Because these gridded archives are large and each retained origin requires seven quantile-specific fits followed by posterior predictive synthesis, \textcolor{red}{using many overlapping forecast origins would require a substantially more data collection, preparation and computation while repeatedly evaluating similar hydrological episodes. We therefore avoid densely overlapping forecast origins. The selected origins are limited by the availability of  compatible archived forecast products, and closely overlapping forecast windows could overrepresent the same hydrological episode.}
...
Figures~\ref{fig:dry_quantile} and \ref{fig:rainy_quantile} show clear differences between posterior dynamics in the drought and wet periods. During 2012--2016, the posterior bands remain comparatively narrow and the upper tail rarely approaches the flood thresholds. During 2017--2019, the upper tail widens substantially during the main winter events, and the 95th quantile exceeds the major flood stage in 2017 and 2019.
...
\label{fig:synth1}
\end{figure}
Because exAL-M-T1 is the selected extended-likelihood multivariate specification, Figure~\ref{fig:synth1} is the main predictive-synthesis illustration. The synthesized predictive distribution combines observed USGS measurements before the cutoff with the retrospective and forecast products available at that origin. The upper quantiles rise sharply after December 25, 2022 and remain above the major flood threshold for several days, reflecting the high flood risk in this forecast window. Unlike the fitted historical quantile-location summaries, the posterior predictive envelope can vary across the forecast window as the source-specific corrections and forecast-window covariates change. A univariate transfer-active reference is reported in Appendix~\ref{app:historicalsynth} to separate the multivariate operational synthesis from a simpler reference that excludes retrospective-product and forecast-product source channels.
