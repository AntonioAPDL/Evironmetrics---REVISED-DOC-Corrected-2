...
Our approach fits a separate state-space model for each target quantile level, which allows the seven quantile-specific models used in the application to be fitted in parallel. In the San Lorenzo application, the full quantile set can be refit on operational time scales once observations, retrospective products, forecast products, and forecast covariates have been staged; we return to this computational point in Section~\ref{sec:forecastvalidation}.
...
For a predictive distribution \(\mathbf{G}\) and an observation \(y\), the CRPS is defined as
\[
\text{CRPS}(y, \mathbf{G}) = 2 \int_0^1 \rho_\tau\!\left(y - \mathbf{G}^{-1}(\tau)\right)\, d\tau,
\qquad
\rho_\tau(u) = u\big(\tau - \mathbf{1}\{u<0\}\big),
\]
where \(\rho_\tau\) is the quantile score, or check loss, and \(\mathbf{G}^{-1}\) is the predictive quantile function. Thus, the CRPS is a natural score in the context of quantile estimation. Assuming that \(\mathbf{G}\) has a finite mean, the CRPS can also be written as
\[
...
For an ensemble forecast with members \(x_1,\ldots,x_M\), we use the empirical CRPS estimator
\[
\widehat{\mathrm{CRPS}}(F_M,y)
= \frac{1}{M}\sum_{m=1}^{M}|x_m-y|
- \frac{1}{2M^2}\sum_{m=1}^{M}\sum_{\ell=1}^{M}|x_m-x_\ell|.
\]
A deterministic forecast is the special case \(M=1\), for which the CRPS reduces to \(|y-x|\), the absolute error. We also use CRPS for the preliminary selection of the transfer-persistence parameter \(\lambda\), as described in Subsection~\ref{subsec:specification}.
...
The transfer component takes three inputs: local precipitation from the PRISM Climate Group \citep{prism}, local soil moisture from ECMWF ERA5-Land \citep{ecmwf}, and a low-dimensional summary of broader climate conditions. Pre-cutoff precipitation and soil moisture are historical gridded covariates extracted at the Big Trees location; after the cutoff, their forecast-window values are staged from the same forecast-origin bundle used to construct the hydrologic forecast matrices. These forecast-window covariates enter as deterministic summaries in the present implementation.
...
\caption{
Univariate transfer-active predictive distribution for river flow on the $\log(1+x)$ scale, with $x$ measured in m$^3$/s, at the December 25, 2022 forecast origin. Observed measurements before the origin are shown in green, held-out USGS verification after the origin is shown in red, and the reference predictive distribution is shown by the pink band and dark center line. The reference uses the USGS observation channel and transfer covariates, but excludes retrospective-product and forecast-product source channels.
}
\label{fig:synth2}
\end{figure}
...
\caption{
Predictive synthesis panel for the January 23, 2021 cutoff. The display follows the same selected-model synthesis and reference-product overlay convention as Figure~\ref{fig:synth1}.
}
...
\caption{
Predictive synthesis panel for the November 12, 2021 cutoff. The display follows the same selected-model synthesis and reference-product overlay convention as Figure~\ref{fig:synth1}.
}
...
\caption{
Predictive synthesis panel for the December 21, 2021 cutoff. The display follows the same selected-model synthesis and reference-product overlay convention as Figure~\ref{fig:synth1}.
}
...
\caption{
Predictive synthesis panel for the May 11, 2022 cutoff. The display follows the same selected-model synthesis and reference-product overlay convention as Figure~\ref{fig:synth1}.
}
