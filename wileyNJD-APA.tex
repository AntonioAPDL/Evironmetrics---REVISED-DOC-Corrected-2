...
\]
A deterministic forecast is the special case \(M=1\), for which the CRPS reduces to \(|y-x|\), the absolute error. In preliminary tuning, the transfer persistence \(\lambda\) was selected by minimizing CRPS for the synthesized predictive distribution; this tuning selected \(\lambda=0.97\), which is then held fixed in the final publication specifications.
