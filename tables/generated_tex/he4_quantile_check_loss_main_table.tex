...
\end{tabular*}
\begin{tablenotes}
\item \textit{Note:} The check loss is computed over the forecast-window rows only, using the held-out USGS observations as the verification targets, on the same $\log(1+Q)$ scale used for CRPS. 
%The four synthesis competitors are resolved directly from the frozen HE-2 publication manifest.
\end{tablenotes}
\end{threeparttable}
\end{table*}
